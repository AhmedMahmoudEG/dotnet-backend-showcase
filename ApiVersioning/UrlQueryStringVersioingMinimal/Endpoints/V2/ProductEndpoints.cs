...
public static class ProductEndpoints
{
    ...
    {
        var productApi = app.MapGroup("/api/v{apiVersion:apiVersion}/products").WithApiVersionSet(apiVersionSet); 

        productApi.MapGet("{productId:guid}",GetProductById).HasApiVersion(new ApiVersion(2)).WithName("GetPRoductByIdV2");
